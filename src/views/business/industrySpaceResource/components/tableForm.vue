...
<script setup lang="ts">
  ...
  type IndustryItem = Industry.IndustryEntering.IndustryListItem
  ...
  const {
    ...
    core: {
      ...
      columnsFactory: () => [
        ...
        { prop: 'industrySpaceNumber', label: '空间资源编号', align: 'center' },
        { prop: 'industrySpaceName', label: '空间资源名称', align: 'center' },
        { prop: 'administrativeRegion', label: '行政区', align: 'center' },
        { prop: 'commercialArea', label: '商业面积（平方米）', align: 'center' },
  ...
  // 编辑用户
  const handleEdit = (row: IndustryItem) => {
   ...
   * 删除用户
   */
  const deleteUser = (row: IndustryItem): void => {
